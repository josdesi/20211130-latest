...
after(async () => {
  ...
  if (company) {
    await Database.table('names').where('company_id', company.id).update({ company_id: null });
  ...
  if (companyWithEmployees) {
    await Database.table('names').where('company_id', companyWithEmployees.id).update({ company_id: null });
    await deleteCompany(companyWithEmployees.id);
  }

  if (contactWithEmployees) {
    await Database.table('names').where('company_id', contactWithEmployees.id).update({ company_id: null });
    await contactWithEmployees.delete();
  }
});
