...
class AddPlacementOverridePermissionSchema extends Schema {
  up () {
  ...
  down () {
    this.schedule(async (transaction) => {
      await transaction.table('permissions')
        .where('id', userPermissions.placements.overrideApproval)
        .delete();
    });
  }
}

module.exports = AddPlacementOverridePermissionSchema
