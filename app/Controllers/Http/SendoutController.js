...
class SendoutController {
  ...
  /**
   *
   * GET sendouts/get-leaders-of-week
   *
   * @param {Response} ctx.response
   *
   * @return {Object} from leaders of the week
   */
  async getLeadersOfWeek({ response }) {
    try {
      const totalLedersInTheWeek = 10;
      const result = await this.sendoutRepository.getTopLeadersOfWeek(totalLedersInTheWeek);
      if (!result.success) throw result.message || `Unexpected error while to get the leaders of the week in sendouts`;
