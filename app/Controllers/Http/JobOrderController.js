...
const { getRadiusFilterResolver, multipleFilterParser, defaultWhereResolver, applyOrderClause, multipleWhereResolver, positionFilterResolver } = (use('App/Helpers/QueryFilteringUtil'));
...
class JobOrderController {
  constructor() {
    this._filterOptionsColumnMap = {
      ...
      radius: {resolver: getRadiusFilterResolver({zipColumn: 'addresses.zip', coordsColumn: 'jo.coordinates'})}
    };
  ...
  async applyWhereClause(queryParams, query, user_id) {
    const filters = {
      ...
      'radius': { radius: queryParams.radius, zips: multipleFilterParser(queryParams.zips)},
    };
