...
const {getRadiusFilterResolver, multipleFilterParser, defaultWhereResolver, multipleWhereResolver} = (use('App/Helpers/QueryFilteringUtil'));
...
class CompanyController {
  constructor() {
    ...
    this._filterOptionsColumnMap = {
      ...
      radius: {resolver: getRadiusFilterResolver({zipColumn: 'cp.zip', coordsColumn: 'cp.coordinates_as_geography'})}
    };
  }
  ...
  async applyWhereClause(request, query, user_id) {
    const filtersFromRequest = request.only([
      'industryId',
      'stateId',
      'cityId',
      'zip',
    ...
    const filters = {
      ...filtersFromRequest,
      radius: { radius: filtersFromRequest.radius, zips: multipleFilterParser(filtersFromRequest.zips) }
    };
