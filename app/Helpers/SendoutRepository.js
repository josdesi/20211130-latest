...
class SendOutRepository {
  ...
  /**
   *
   * @param {Integer} totalLedersInTheWeek
   * @returns {Array} of leaders of the week
   */
  async getTopLeadersOfWeek(totalLedersInTheWeek = 10) {
    try {
      const leaders = await Database.table('v_weekly_sendout_board')
        .select(['recruiter_id', 'recruiter', 'total'])
        .where('tota', '>=', minimumForDailyLeaders)
        .orderBy('total', 'desc')
        .limit(totalLedersInTheWeek);
   ...
   *
   * @param {*} userId
   * @returns {Object} My total sendout of the week
   */
  async getMyTotalSendoutOfWeek(userId) {
    return await Database.table('v_weekly_sendout_board')
      .select(['recruiter_id', 'recruiter', 'total', 'goal'])
   ...
   *
   * @param {*} userId
   * @returns {Object} My total sendover converted of the week
   */
  async getMyTotalSendoverOfWeek(userId) {
    const query = `with weekly_sendovers as (select
        count(so.id) as sendover,
        SUM(case when so.converted = true then 1 else 0 end) as converted
      from
        sendouts as so
      left join v_users as rejo on
        so.job_order_accountable_id = rejo.id
      left join v_users as reca on
        so.candidate_accountable_id = reca.id
      where
        deleted = false
        and so.board_date::date >= date_trunc('week', now())
        and so.board_date::date <= timezone('US/Central'::text, now())::date
        and (reca.id = ${userId} or rejo.id = ${userId})
      group by
        so.sendout_status_id)
      select sum(converted) as converted, sum(sendover) as sendover from weekly_sendovers;
    `;
