...
class SendOutRepository {
  ...
  async getListing(request, userId, timezone, columnsToSelect, withPaginate, groupBy, withOrder) {
    ...
    if (rest.userFilter && Number(rest.userFilter) === userFilters.MyInventory) {
      query
        .leftJoin('job_order_additional_recruiters as jo_ar', 'jo.id', 'jo_ar.job_order_id')
        .leftJoin('candidate_additional_recruiters as ca_ar', 'ca_ar.candidate_id', 'ca.id');
    }
  ...
  async multipleRegionalResolver({ query, value }) {
    ...
    query.where(function () {
      this.whereIn(`rejo.regional_id`, whereInClauseRecruiters);
    });
  }
  ...
  async multipleCoachResolver({ query, value }) {
    ...
    query.where(function () {
      this.whereIn(`rejo.coach_id`, whereInClauseRecruiters);
    });
  }
  ...
  async getPayloadOperation(sendout, interview, userId) {
    ...
    return Env.get('REMINDER_SENDOUT') === 'prod' ? distributionEmails : distributionEmailsTest;
  }
  ...
  async createEmailToOperationsSendout(sendoutId, interview, req, userId) {
    ...
    await SendoutEmail.sendPersonalizedEmail(payload, SendoutTypesEmails.SendoutNotification);
  ...
  async createEmailToOperationsSendover(sendoutId, req, userId) {
    ...
    await SendoutEmail.sendPersonalizedEmail(payload, SendoutTypesEmails.SendoverNotification);
  ...
  async createReminderToHiringAuthority(sendoutId, interview, req, userId, newReminder) {
    ...
    if (reminders.length > 0) {
      ...
      if (newReminder) await SendoutEmail.sendPersonalizedEmail(payload, SendoutTypesEmails.Sendout);
  ...
  calculateSummaryResult(status, sendouts, summaryData) {
    ...
    const ratioPlaced = summaryData.placed ? Math.round(totalSendouts / summaryData.placed) : 0;
