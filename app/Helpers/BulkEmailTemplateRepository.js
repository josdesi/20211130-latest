...
class BulkEmailTemplateRepository {
    ...
    return query.with(`childrenFolders`, (builder) => {
      builder
        ...
        .with('emailTemplates', (_builder) => {
          ...
          this.addScopeQuery(_builder, scopeId);
        });
  ...
  async checkUserDefaultFolders(userId, trx = null) {
    try {
      const userHasFolders = await EmailTemplateFolder.query()
        .where('created_by', userId)
        .where('is_default_folder', true)
        .count();

      if (userHasFolders[0].count >= 2) {
        return {
          success: true,
  ...
  async getSharedTemplatesFoldersWithTemplates(keyword, user_id, scopeId) {
    let sharedTemplatesFolders = EmailTemplateFolder.query()
      ...
      .with('emailTemplates', (builder) => {
        ...
        this.addScopeQuery(builder, scopeId);
      });
  ...
  /**
   * Applies a filter to the emailTemplates / bulkEmailTemplates by their scope id
   *
   * @summary Removes any template that does not contain the scope desired, if none is passed, returns those templates that at least have one scope
   *
   * @param {Object} query - The builder from the model, expected to be called only from the bulkEmailTemplates model
   * @param {Number} scopeDesired - The id of the scope desired
   */
  addScopeQuery(query, scopeDesired = null) {
    if (scopeDesired) {
      query.whereHas('bulkType', (builder) => {
        builder.where('id', scopeDesired);
      });
    } else {
      query.whereHas('bulkType');
    }
  }
      ...
      const emailTemplate = await EmailTemplate.create(
        {
          ...
          bulk_email_scope_type_id: templateData.bulk_email_scope_type_id,
        },
        trx
      );
  ...
  async details(id, user_id) {
    try {
      ...
      const parentFolderTree = await this.getParentFolderTree(
