...
class SearchProjectInventoryRepository {
   ...
   *
   */
  getSearchProjectItemsQuery(searchProjectId) {
    const candidatesQuery = this.getCandidatesQuery(searchProjectId);
    const haQuery = this.getHiringAuthorityQuery(searchProjectId);
    const nameQuery = this.getNameQuery(searchProjectId);
   ...
   *
   */
  getCandidatesQuery(searchProjectId) {
   ...
   *
   */
  getHiringAuthorityQuery(searchProjectId) {
  ...
  async deleteSearchProjectInventory(itemsToRemove, searchProjectId, trx, userId) {
    try {
      ...
      const inventoryIds = await this.getInventoryByParams(searchParams, searchProjectId, userId);
  ...
  async getInventoryByParams({ query, exclude = [] }, searchProjectId, userId) {
    const ids = {
      names: [],
      candidates: [],
      ha: [],
