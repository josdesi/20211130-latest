...
class SearchProjectRepository {
  ...
  async getItemsByQueryParams(queryParams = {}, userId) {
    ...
    if (searchProjectQuery && searchProjectId) {
      const searchProjectSelection = { params: searchProjectQuery };
  ...
  reportInvalidItemIds(invalidItemIds = [], selectedItemIds = [], type = '') {
    const properties = {
      invalidItemIds,
      selectedItemIds,
      type,
    ...
    appInsights.defaultClient.trackEvent({ name: 'Search Project Creation Failed', properties });
  }
  ...
  async validateSearchProjectCreation(candidates, hiringAuthorities, names) {
    ...
    if (candidates && candidates.length > 0) {
      ...
      if (!candidateValidation.success) {
        this.reportInvalidItemIds(candidateValidation.data, candidates, 'candidates');
        return {
          success: false,
          code: 400,
          message: 'You selected an invalid Candidate',
        };
      }
    ...
    if (hiringAuthorities && hiringAuthorities.length > 0) {
      ...
      if (!hiringAuthorityValidation.success) {
        this.reportInvalidItemIds(hiringAuthorityValidation.data, hiringAuthorities, 'hiringAuthorities');
        return {
          success: false,
          code: 400,
          message: 'You selected an invalid Hiring Authority',
        };
      }
    ...
    if (names && names.length > 0) {
      ...
      if (!namesValidation.success) {
        this.reportInvalidItemIds(namesValidation.data, names, 'names');
        return {
          success: false,
          code: 400,
          message: 'You selected an invalid Name',
        };
      }
