...
class MicrosoftGraph {
  ...
  async getEmail(subscriptionId, messageId) {
    try {
      const { client, userId } = await this.getClientBySubscriptionId(subscriptionId);
      if (!client)
        return {
          success: false,
          message: Antl.formatMessage('messages.error.microsoftgraph.logOut', { service: 'mailing service' }),
      ...
      const emailPayload = {
        ...
        sender: {
          address: String(response.sender.emailAddress.address).formatToCompare(),
  ...
  async storeMail(emailPayload) {
    ...
    try {
      ...
      const allParticipants = [...formatedSender, ...recipients, ...ccRecipients];
  ...
  getEmailTrackingActivityMessage(sender, recipients, ccRecipients, subject) {
    const messageFormatedRecipients = recipients
      .map((row) => `${row.emailAddress.name} &lt;${String(row.emailAddress.address).formatToCompare()}&gt;`)
      .join(', ');
  ...
  async validateEmailForTracking(sender, recipients, ccRecipients) {
    const formatedSender = String(sender).formatToCompare();
    const formatedRecipients = recipients.map(({ emailAddress }) => String(emailAddress.address).formatToCompare());
    const formatedCCRecipients = ccRecipients.map(({ emailAddress }) => String(emailAddress.address).formatToCompare());

    const allEmails = [formatedSender, ...formatedRecipients, ...formatedCCRecipients];
    const blockedEmails = await Database.table('email_tracking_block_lists').whereRaw(
      `LOWER(email) IN ${joinStringForQueryUsage(allEmails)}`
    );

    const blockedFromEmails = blockedEmails.filter((row) => row.block_from === true).map((row) => row.email);
    const blockedToEmails = blockedEmails.filter((row) => row.block_to === true).map((row) => row.email);

    if (blockedFromEmails.includes(formatedSender)) {
      return {
        success: false,
    ...
    if (formatedRecipients.some((email) => blockedToEmails.includes(email))) {
      return {
        success: false,
    ...
    if (formatedCCRecipients.some((email) => blockedToEmails.includes(email))) {
      return {
        success: false,
        message: 'One CC recipient is in our email tracking block list',
        data: blockedEmails,
      };
    }
  ...
  async findInventory(recipients) {
    const emails = recipients.map((row) => String(row.emailAddress.address).formatToCompare());
