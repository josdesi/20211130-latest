...
class BulkEmailRepository {
   ...
   * @param {Object} emailData.search_params - This contains the search project params used when sending the bulk
   * @param {Object} emailData.search_candidate_ids - This contains the search project selected candidates
   * @param {Object} emailData.search_hiring_authority_ids - This contains the search project selected hirings
   * @param {Object} emailData.search_name_ids - This contains the search project selected names
   *
  ...
  async createAndSend(emailData, userId, candidateIds = [], jobOrderIds = []) {
    ...
    try {
      ...
      const searchProjectSelection = this.getSearchProjectSelectionJSON(emailData);
      ...
      const bulkData = await this.createBulk(emailData, userId, bulkConfiguration, trx);
      const { emailBody, attachments } = bulkData;

      const sentData = await this.sendBulk(userId, bulkConfiguration, bulkData, trx);
      if (!sentData.success) {
        await trx.rollback();
        return sentData;
      }
  ...
  /**
   * Obtains the search proejct selection config from the email data
   *
   * @summary This method allows to quickly obtain the search project selection config; Since it has a lot of usage in more that one method, the logic of declaring & obtaining it can be modularized
   *
   * @param {Object} emailData - Check out @method createAndSend for more information on this object, but we are interested on search_params, search_candidate_ids, search_hiring_authority_ids & search_name_ids
   *
   * @return {Object} A JSON object containing the params, candidateIds, hiringAuthorityIds & nameIds from the emailData
   */
  getSearchProjectSelectionJSON(emailData = {}) {
    const searchProjectSelectionParams = emailData.search_params ? emailData.search_params : {};
    const searchProjectSelectionCandidateIds = emailData.search_candidate_ids ? emailData.search_candidate_ids : [];
    const searchProjectSelectionHiringAuthorityIds = emailData.search_hiring_authority_ids
      ? emailData.search_hiring_authority_ids
      : [];
    const searchProjectSelectionNameIds = emailData.search_name_ids ? emailData.search_name_ids : [];
   ...
   */
  async validateTemplateAndDraft(templateId, draftId, searchProjectId, userId, blockResendObject) {
    if (draftId) {
    ...
    if (templateId) {
      ...
      const isTemplateValid = await this.templateValidation(templateId);
      if (!isTemplateValid.success) return isTemplateValid;
   ...
   * @param {Object} emailData - Check out @method createAndSend for more information on this object
   * @param {Number} userId - Who is sending the bulk email
   * @param {Number[]} bulkConfiguration - Contains the bulk configuration, params that are usually optional & modifies the bulk behaviour
   * @param {Number[]} bulkConfiguration.candidateIds - The candidates which are being market-ed, usually only passed when the email has a marketing scope
   * @param {Number[]} bulkConfiguration.jobOrderIds - The job orders which are being used for recruiting purposes, usually only passed when the email has a recruiting scope
   * @param {Object} bulkConfiguration.searchProjectSelection - The search project params selected by the user when sending the bulk
   ...
   */
  async createBulk(emailData, userId, bulkConfiguration, trx) {
    const { candidateIds = [], jobOrderIds = [], searchProjectSelection = {} } = bulkConfiguration;
    ...
    const emailHistory = await EmailHistory.create(
      {
        ...
        search_project_selection_params: searchProjectSelection,
      },
      trx
    );
  ...
  async createFailureDraft(emailData, userId, candidateIds = [], jobOrderIds = [], errors) {
    ...
    try {
      ...
      const { emailBody, emailHistory, attachments } = await this.createBulk(emailData, userId, bulkConfiguration, trx);
   ...
   * @param {Number[]} bulkConfiguration.candidateIds - The candidates which are being market-ed, usually only passed when the email has a marketing scope
   * @param {Number[]} bulkConfiguration.jobOrderIds - The job orders which are being used for recruiting purposes, usually only passed when the email has a recruiting scope
   * @param {Object} bulkConfiguration.blockedByResendItems - An object containing the items blocked by the resend function
   * @param {Object} bulkConfiguration.searchProjectSelection.params - The params that the search project will be applied to, allowing to select an specific output of the SP
   * @param {Object} bulkConfiguration.searchProjectSelection.candidateIds - This contains the search project selected candidates
   * @param {Object} bulkConfiguration.searchProjectSelection.hiringAuthorityIds - This contains the search project selected hirings
   * @param {Object} bulkConfiguration.searchProjectSelection.nameIds - This contains the search project selected names
   * @param {Database.model} bulkData.emailBody - The EmailBody model, This method requires the database object, DO NOT JSON()'d IT BEFORE PASSING IT HERE
   * @param {Database.model} bulkData.emailHistory - The EmailHistory model, This method requires the database object, DO NOT JSON()'d IT BEFORE PASSING IT HERE
   * @param {Object[]} bulkData.attachments - An array of objects of the type Attachments model, its origin should be by the JSON() method or the .with('attachments') from the emailHistory, no need for the Database model
   * @param {Database} trx - External transaction object, the transaction should be controlled out side this method, since this method could be part of a bigger scope
   *
   ...
   */
  async sendBulk(userId, bulkConfiguration, bulkData, trx) {
    const { candidateIds, jobOrderIds, blockedByResendItems, searchProjectSelection } = bulkConfiguration;
    const { emailBody, emailHistory, attachments } = bulkData;
    ...
    const searchProjectEmails = await this.getSearchProjectEmails(searchProjectId, searchProjectSelection, userId);
  ...
  async destroyDraft(id, user_id, destroyAttachments = true, externalTrx = null) {
    ...
    try {
      ...
      await BulkEmailFailureMessage.query().transacting(trx).where('email_history_id', emailDraft.id).delete();
  ...
  /**
   * This guard method checks if the template is valid
   *
   * @description This method is expected to be called in the create bulk method, before this method was modified, the scope was checked, but now it is enought by knowing that it exists
   *
   ...
   *
   */
  async templateValidation(templateId) {
    const emailTemplate = await EmailTemplate.query().whereHas('emailBody').where('id', templateId).first();
  ...
  async getSearchProjectEmails(searchProjectId, searchProjectSelection = {}, userId = null) {
    const result = await SearchProjectInventoryRepository.getInventoryDetails(
      searchProjectId,
      searchProjectSelection,
      userId
    );
