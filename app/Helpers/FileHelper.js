...
const moveFile = async (fileName, path) => {
  ...
  try {
    await Drive.disk('azure').move('tmp/'+ decodeURL(fileName), decodedPath);
  } catch (error) {
    throw error;
  }
...
const getFileBuffer = async (url) => {
  const pathFile = urlP.parse(decodeURL(url)).path.split('/files/');
  const urlPath = pathFile[1].split('/');
  const path = urlPath.join('/')

  return await Drive.disk('azure').get(decodeURL(path))
};
...
const splitBlobUrlByContainer = (url, defaultContainer = 'files') => {
  ...
  const urlParts = decodeURL(url).split(`/${defaultContainer}/`);
...
/**
 * Decodes the URL of a file
 * @param {String} url Url of File 
 * @returns {String} the url decoded or as it was
 */
const decodeURL = (url) => {
  try {
