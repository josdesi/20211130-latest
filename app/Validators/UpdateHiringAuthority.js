...
class UpdateHiringAuthority extends BaseValidator {
  get rules() {
    const hiringAuthorityId = this.ctx.params.hiringAuthorityId ? this.ctx.params.hiringAuthorityId : this.ctx.params.id;
    ...
    return {
      ...
      personal_email: `email|max:255|uniqueCaseInsensitive:hiring_authorities,personal_email,id,${hiringAuthorityId}`,
      personal_phone: 'integer|min:10|max:10',
      work_email: `required|email|max:255|uniqueCaseInsensitive:hiring_authorities,work_email,id,${hiringAuthorityId}`,
  ...
  get messages() {
    return {
      ...
      'personal_email.email': 'Please enter a valid email address {{value}}',
...
module.exports = UpdateHiringAuthority
