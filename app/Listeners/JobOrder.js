...
const JobOrderSearchInformationUpdater = new (use('App/Helpers/Search/Updater/JobOrderSearchInformationUpdater'))();
...
const extractId = ({id, jobOrderId, jobOrder, job_order_id}) => {
  if (id) return id;
  if (jobOrderId) return jobOrderId;
  if (job_order_id) return job_order_id;
  if (jobOrder && jobOrder.id) return jobOrder.id;
};
...
const JobOrder = module.exports = { 
  ...
  updateSearchInformation: async(params) => {
    try {
      const id = extractId(params);
      if(id) {
        await JobOrderSearchInformationUpdater.updateSearchInformationByIds(id);
      }
    } catch(error) {
      appInsights.defaultClient.trackException({ exception: error });
    }
