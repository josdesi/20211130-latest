...
const CandidateSearchInformationUpdater = new (use('App/Helpers/Search/Updater/CandidateSearchInformationUpdater'))();
...
const Candidate = module.exports = {
  ...
  updateOrCreateDirectoryInformation: async(params) => {
    try {
      ...
      if(id) {
        await CandidateDirectoryUpdater.updateOrCreateDirectoryInformation(id);
      }
  ...
  updateSearchInformation: async(params) => {
    try {
      ...
      if (!id && candidateIds.length === 0) return;
      if(id) {
        await CandidateSearchInformationUpdater.updateSearchInformationByIds(id);
      }
      if(candidateIds.length > 0){
        for(const _id of candidateIds){
          await CandidateSearchInformationUpdater.updateSearchInformationByIds(_id);
        }
      }
    } catch(error) {
      appInsights.defaultClient.trackException({ exception: error });
