...
Route.group(() => {
  ...
  .middleware(['auth:jwt', 'statusActive','hasRole:Data Coordinator'])
  .prefix('api/v1/migrations');

Route.group(() => {
  Route.get('/templates', 'PCRController.templates');
  Route.get('/users', 'PCRController.users');
  Route.post('/templates/map/tags', 'PCRController.mapTags');
  Route.post('/import/templates', 'PCRController.importTemplate').validator('ImportBulkEmailTemplate');
})
  .middleware(['auth:jwt', 'statusActive','hasRole:Data Coordinator'])
  .prefix('api/v1/pcr');
